...
;;; T&iacute;tulo: Unification as transformation rules (using a prefix representation of terms)
;;; ============================================================================
...
(certify-book "prefix-unification-rules")
...
;;; two ACL2 functions. A function {\tt unif-legal-p}, checking if it is
...
(defun unif-legal-p-decompose (t1 t2)
...
(defun unif-legal-p-clash1 (t1 t2)
  (and (not (variable-p t1))
       (not (variable-p t2))
       (not (equal (car t1) (car t2)))))

(defun unif-legal-p-clash2 (t1 t2)
  (and (not (variable-p t1))
       (not (variable-p t2))
       (mv-let (pair-args bool)
	       (pair-args (cdr t1) (cdr t2))
  ...
  (let ((name (first op))
    ...
    (and (natp equ-n)
	 ...
	 (let* ((equ (nth equ-n tbs))
		...
		(t2 (cdr equ)))
	   (case name
		 (delete (unif-legal-p-delete t1 t2))
		 (decompose (unif-legal-p-decompose t1 t2))
		 (orient (unif-legal-p-orient t1 t2))
		 (eliminate (unif-legal-p-eliminate t1 t2))
		 (clash1 (unif-legal-p-clash1 t1 t2))
		 (clash2 (unif-legal-p-clash2 t1 t2))
		 (occur-check (unif-legal-p-occur-check t1 t2))
		 (t nil))))))
...
(defun unif-reduce-one-step-p-decompose (equ R sol)
...
(defun unif-reduce-one-step-p-clash1 (equ R sol)
  (declare (ignore equ R sol))
  nil)

(defun unif-reduce-one-step-p-clash2 (equ R sol)
  (declare (ignore equ R sol))
  nil)

(defun unif-reduce-one-step-p-orient (equ R sol)
  (let ((t1 (car equ)) (t2 (cdr equ)))
    (list (cons (cons t2 t1) R) sol)))


(defun unif-reduce-one-step-p-eliminate (equ R sol)
  (let ((sigma (list equ)))
...
(defun unif-reduce-one-step-p (S-sol op)
  (let* ((name (first op))
	 (equ-n (second op))
	 (S (first S-sol))
	 ...
	 (equ (nth equ-n S)))
    (case name
      (delete (unif-reduce-one-step-p-delete equ R sol))
      (decompose (unif-reduce-one-step-p-decompose equ R sol))
      (orient (unif-reduce-one-step-p-orient equ R sol))
      (eliminate (unif-reduce-one-step-p-eliminate equ R sol))
      (clash1 (unif-reduce-one-step-p-clash1 equ R sol))
      (clash2 (unif-reduce-one-step-p-clash2 equ R sol))
      (occur-check (unif-reduce-one-step-p-occur-check equ R sol))
      (t nil))))
...
; 		  (solution sigma (both-systems (unif-reduce-one-step-p S-sol
; 			 					  op))))
; 	     (solution sigma (both-systems S-sol))))

; (defthm mm-preserves-solutions-3
;   (implies (and (unif-legal-p S-sol op)
; 		  (not (unif-reduce-one-step-p S-sol op)))
...
;;; We show now that we can define a lexicographic measure on pair of
;;; systems of equations and prove that this measure decreases in every
;;; single step of transformation. The measure is defined in {\tt
;;; terms.lisp} and is given by the function {\tt
;;; unification\--measure}:
...
; 	     (o< (unification-measure (unif-reduce-one-step-p S-sol op))
; 		 (unification-measure S-sol))))

; These two theorems will allow us to use unification-measure to show
...
;; First, established in terms of @subsetp@:
  ...
  (defthm unif-reduce-one-step-p-does-not-add-new-variables
    (implies (unif-legal-p S-sol op)
...
(local
 (defthm
   ...
   (implies (and (unif-legal-p S-sol op)
		...
		(size-system (first (unif-reduce-one-step-p S-sol op)))))
   :rule-classes :linear
...
(local
 (defthm
   ...
   (implies (and (unif-legal-p S-sol op)
		...
		(first (unif-reduce-one-step-p S-sol op)))
	       (n-variables-right-hand-side (first S-sol))))
  :rule-classes :linear))
...
(defun unif-seq-p-p (S-sol unif-seq)
  (declare (xargs :measure (acl2-count unif-seq)))
  (if (endp unif-seq)
      t
    (let ((op (car unif-seq)))
      (and (unif-legal-p S-sol op)
	   (unif-seq-p-p
	    (unif-reduce-one-step-p S-sol op)
	    (cdr unif-seq))))))
...
(defun unif-seq-p-last (S-sol unif-seq)
  (if (endp unif-seq)
      S-sol
    (unif-seq-p-last (unif-reduce-one-step-p S-sol (car unif-seq))
		     (cdr unif-seq))))
...
(local
 (defthm if-solvable-transform-sel-success
   (implies (and (consp seq)
	    ...
	    (unif-reduce-one-step-p S-sol (car seq)))
   :hints (("Goal" :in-theory (enable unif-legal-p)))))
 ...
 (defthm unif-seq-p-preserves-idempotency
   (let* ((S (first S-sol))  (sol (second S-sol))
      ...
      (implies (and (unif-seq-p-p S-sol unif-seq)
		    ...
		    (disjointp (system-var S) (domain sol)))
	       (idempotent solution)))
   :hints (("Goal" :induct (unif-seq-p-last S-sol unif-seq)
...
(defun mgs-seq-p (S seq)
  (and (unif-seq-p-p (list S nil) seq)
       (normal-form-syst (unif-seq-p-last (list S nil) seq))))

(defun mgs-seq (S seq)
  (unif-seq-p-last (list S nil) seq))
...
(defthm mgs-seq-most-general-solution-main-lemma
  ...
  :hints (("Goal" :use
	   (:instance
	    unif-seq-p-equivalent-1  (S-sol (list S nil))))))
